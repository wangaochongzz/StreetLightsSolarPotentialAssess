...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": 'logs/ep033-loss0.032-val_loss0.031.pth',
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        with torch.no_grad():
            ...
            if results[0] is None:
                return image, predicted_classes, predicted_lefts, predicted_rights, predicted_tops, predicted_bottoms
        ...
        # ---------------------------------------------------------#
        #   在图像绘制前，删除掉重叠的预测结果(SL_1)
        # ---------------------------------------------------------#
        def boxes_overlap(box1, box2):
            """
            判断两个边界框是否发生重叠
            box1 和 box2 分别是 (top, left, bottom, right) 格式的坐标
            """
            return not (box2[1] > box1[3] or box2[3] < box1[1] or box2[0] > box1[2] or box2[2] < box1[0])

        # 遍历检测到的SL_1和SL_2的预测结果
        sl1_indices = [i for i, label in enumerate(
            top_label) if self.class_names[label] == "SL_1"]
        sl2_indices = [i for i, label in enumerate(
            top_label) if self.class_names[label] == "SL_2"]

        # 记录需要删除的 SL_1 预测索引
        indices_to_delete = []

        # 遍历 SL_1 和 SL_2 的索引，删除重叠的 SL_1 预测
        for idx1 in sl1_indices:
            for idx2 in sl2_indices:
                # 检查索引是否在列表的有效范围内
                if 0 <= idx1 < len(top_label) and 0 <= idx2 < len(top_label):
                    # 获取两个预测的边界框
                    box1 = top_boxes[idx1]
                    box2 = top_boxes[idx2]
                    ...
                    if boxes_overlap(box1, box2):
                        # 记录需要删除的 SL_1 预测索引
                        indices_to_delete.append(idx1)
                        break

        # 一次性删除需要删除的索引对应的元素
        for idx in reversed(indices_to_delete):
            top_label = np.delete(top_label, idx, axis=0)
            top_conf = np.delete(top_conf, idx, axis=0)
            top_boxes = np.delete(top_boxes, idx, axis=0)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            print(label, left, right, top, bottom)

            predicted_classes.append(predicted_class)
            predicted_lefts.append(left)
            predicted_rights.append(right)
            predicted_tops.append(top)
            predicted_bottoms.append(bottom)
