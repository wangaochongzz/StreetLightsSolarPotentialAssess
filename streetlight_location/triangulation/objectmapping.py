...
# Input CSV file
inputfilename = r'E:\location_test\test718_detected_bearingsecond_depth_2000.csv'
# Output CSV file
outputfilename = r'E:\location_test\test718_detected_bearingsecond_depth_tri_2000_222.csv'

# preset parameters
MaxObjectDstFromCam = 20  # Max distance from camera to objects (in meters)
MaxDstInCluster = 3  # Maximal size of clusters employed (in meters)

# MRF optimization parameters
ICMiterations = 50  # Number of iterations for ICM
...
# 定义一个函数来计算GPS位置
def calculate_gps_location(camera_lat, camera_lon, bearing, depth):
    bearing_rad = radians(bearing)
    earth_radius = 6371000
    delta_lat = (depth / earth_radius) * cos(bearing_rad)
    delta_lon = (depth / earth_radius) * sin(bearing_rad)
    new_lat = camera_lat + (delta_lat * 180.0 / pi)
    new_lon = camera_lon + (delta_lon * 180.0 / pi)
...
def haversine(lon1, lat1, lon2, lat2):
    ...
    return m
'''
这段代码定义了一个函数 Intersect，用于计算两条射线的交点。以下是函数的输入和输出：

输入：
Object1 和 Object2: 分别是两个对象的信息，包括纬度 (lat)、经度 (lon)、相对北向的偏航角 (bearing)、深度估计 (depth) 等。
MaxObjectDstFromCam: 指定的最大相机到对象的距离。
输出：
如果两条射线没有交点，返回值为 -1, -1, 0, 0。
如果交点在射线之间但在相机后方，返回值为 -2, -2, 0, 0。
如果交点在射线之间但超过了最大距离，返回值为 -3, -3, 0, 0。
如果有有效交点，返回值为 x, y, mx, my，其中 (x, y) 是相交点的坐标，(mx, my) 是投影到米制坐标系的坐标。
这个函数通过射线的两个点和相机的两个点的坐标，计算了两条射线在平面上的交点，同时也考虑了一些特殊情况，例如射线平行的情况。函数通过返回合适的数值表示交点或无效情况。
'''
